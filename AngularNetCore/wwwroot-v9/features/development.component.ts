...
import { BuildConfiguration, VisualProject, AngularProject, BuildResponse, BuildTypes } from 'ngx-modeling';
...
export class DevelopmentBuildDialogComponent {
  buildDialogData: BuildDialogData;
...
export class DevelopmentRemoveDialogComponent {
  removeDialogData: RemoveDialogData;
  constructor(@Inject(MAT_DIALOG_DATA) public data: any) {
...
export class DevelopmentComponent implements OnInit {
  ...
  constructor(readonly bc: BuildConfig, readonly ac: AppConfig, private readonly dialog: MatDialog, private readonly es: EntityService) {
  }

  getBuildTypes() {
    return Object.keys(BuildTypes).filter(x => isNaN(Number(x)));
  }
  ...
  // project management
  onClickDebugRelease(vsProject: VisualProject) {
  ...
  willExecuteRelease(vsProject: VisualProject): string {
